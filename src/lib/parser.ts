...
const MODEL_CONFIGS: { [key: string]: NoteFormatConfig } = {
    ...
    'Omissão de Palavras': { // Texto cloze
...
export function parseNotesFromCSVText(
    ...
    if (!config) {
        throw new Error(`Configuração não encontrada para o Tipo de Nota: "${modelName}".`);
    }
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Usamos uma expressão regular com lookahead negativa para dividir apenas 
        // em `;` que não está dentro de aspas (caso o conteúdo do campo tenha ';').
        // No entanto, para simplicidade e segurança, o AnkiConnect geralmente espera que 
        // o delimitador não apareça no conteúdo. Manteremos o split simples por enquanto.
        ...
        if (parts.length < minFields || parts.length > maxParts) {
             ...
             const expectedFormat = allowOptionalTags 
                ? `${fieldNames.join(';')};[Tags Opcionais]` 
                : fieldNames.join(';');
            
            throw new Error(
                `Erro na Linha ${i + 1}: Formato inválido para "${modelName}". Esperado ${expectedParts} campos. Formato: ${expectedFormat}. Encontrado ${parts.length} campos.`
            );
        }
        ...
        const previewBack = (minFields > 1 && fieldNames[1]) ? noteFields[fieldNames[1]] : '';
